...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f) + 10, label_size.y + style.FramePadding.y * 2.0f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    it_nameanim->second *= (style.Alpha * 1.0215);

    window->DrawList->AddRect(check_bb.Min + ImVec2(4, 4), check_bb.Max + ImVec2(12, -4), ImColor(100, 100, 100), 2);

    window->DrawList->AddRectFilled(check_bb.Min + ImVec2(7, 5) + ImVec2(it_nameanim->second * 11, + 2), check_bb.Max + ImVec2(it_nameanim->second * 11 - 10, -7), ImColor( 71, 160, 185 ), 2);

    if (label_size.x > 0.0f) RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x + 12, check_bb.Min.y + style.FramePadding.y + 1), label);
...
bool ImGui::BeginCombo( const char* label, const char* preview_value, ImGuiComboFlags flags )
{
    ...
    ImRotateStart( );
    RenderTextClipped( bb.Min + style.FramePadding + ImVec2( 110, 1 ), ImVec2( value_x2 + 50, bb.Max.y ), ICON_FA_CHEVRON_UP, NULL, NULL );
...
bool ImGui::SliderScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, float size )
{
    ...
    const ImRect frame_bb( window->DC.CursorPos, window->DC.CursorPos + ImVec2( size, (label_size.y + style.FramePadding.y * 2.0f) - 5 ) );
    ...
    window->DrawList->AddRectFilled( ImVec2( frame_bb.Min.x + 0, frame_bb.Min.y + 9 ), ImVec2( total_bb.Min.x + size, total_bb.Min.y + 12 ), ImColor( 30, 30, 30 ), 0, 0 );
    window->DrawList->AddRectFilled( ImVec2( frame_bb.Min.x - 1, frame_bb.Min.y + 9 ), ImVec2( frame_bb.Min.x + it_filing->second.value + 8, total_bb.Min.y + 12 ), ImColor( 71, 160, 185) );
    ...
    window->DrawList->AddCircleFilled( ImVec2( frame_bb.Min.x + it_filing->second.value + 5, grab_bb.Min.y + 8.0f ), it_namemark->second, ImColor( 71, 160, 185 ), 24 );
    ...
    if ( IsItemActive( ) )
        RenderTextClipped( ImVec2( total_bb.Max.x - 1.5f, total_bb.Min.y ), ImVec2( total_bb.Max.x + 25, total_bb.Max.y ), value_buf, value_buf_end, NULL, ImVec2( 0.7f, 0.5f ) );
    ImGui::PopStyleColor( );
    if ( label_size.x > 0.0f ) RenderText( ImVec2( frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y - 3.0f ), label );
...
{
    return SliderScalar(label, ImGuiDataType_Float, v, &v_min, &v_max, format, flags, size);
}
...
{
    return SliderScalar(label, ImGuiDataType_S32, v, &v_min, &v_max, format, flags, size);
}
